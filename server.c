...
int main(int argc, char **argv) {
    ...
    // set default port and folder
    opt.port = 21;
    strcpy(opt.root, "/Users/amanns/tmp");
    ...
    //if dir change from parameter aply it
    if (!ifchdir) {
        chdir(opt.root);
    }
    ...
    while (1) {
       ...
       // check if server was able to make a new Thread for the client connection.
        if (pid < 0) {
...
void ftp_retr(char *fname, struct server_ctx* context) {
    ...
    if (sendfile(datasfd, fd, 0, &fst.st_size, NULL, 0) == -1 ) {
        send_msg(context->cmd_fd, "425 could not send file\r\n");
    }
